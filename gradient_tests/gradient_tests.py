        ...
        for device in DEVICES:
            for method in METHODS:
                ...
                with self.subTest(device=device, method=method):
                    ...
                    func = lambda y0, t_points: torch_gq_adjoint.odeint(f, y0, t_points, method=method)
            ...
            for ode in PROBLEMS:
                for t_grad in (True, False):
                    ...
                    with self.subTest(device=device, ode=ode, t_grad=t_grad):
                        ...
                        ys = torch_gq_adjoint.odeint(f, y0, t_points, rtol=1e-9, atol=1e-12)
                        torch.manual_seed(0)
                        ...
                        ys = torch_gq_adjoint.odeint_adjoint_gq(f, y0, t_points, rtol=1e-9, atol=1e-12, gtol=0.2)
                        f.nfe = 0
                        ys.backward(gradys)
                        f.nfe = 0
...
class TestCompareAdjointGradient(unittest.TestCase):
    ...
    def test_against_dopri5(self):
        method_eps = {'dopri5': (3e-4, 1e-4, 2e-3)}
        for device in DEVICES:
            for method, eps in method_eps.items():
                for t_grad in (True, False):
                    with self.subTest(device=device, method=method):
                        ...
                        ys = torch_gq_adjoint.odeint_adjoint_gq(func, y0, t_points, method=method, gtol=0.2)
                        func.nfe = 0
                        ...
                        func, y0, t_points = self.problem(device=device)
                        ys = torch_gq_adjoint.odeint(func, y0, t_points, method='dopri5')
                        ys.backward(gradys)
